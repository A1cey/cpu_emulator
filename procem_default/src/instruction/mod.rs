...
impl<W: Word> InstructionTrait<W> for Instruction<W> {
    ...
    fn execute<const STACK_SIZE: usize, P: Deref<Target = [Self]>>(
        ...
        use Instruction::{Add, Call, Dec, Div, Inc, Jump, Mov, Mul, Nop, Pop, Push, Ret, Sub};

        match instruction {
            Nop => (),
            Mov { to, from } => Self::mov(to, from, processor),
            Push { from } => Self::push(from, processor),
            Pop { to } => Self::pop(to, processor),
            Call { addr } => Self::call(addr, processor),
            Ret => Self::ret(processor),
            ...
            Jump { to, condition } => Self::jmp(to, condition, processor),
